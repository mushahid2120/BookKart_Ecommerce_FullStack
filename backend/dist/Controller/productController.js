...
export async function createProduct(req, res, next) {
    const userId = req.id;
    try {
        const { title, category, condition, classType, subject, price, author, edition, description, finalPrice, shippingCharge, paymentMode, paymentDetails, } = req.body;
        const paymentDetailsParse = typeof paymentDetails === "string"
        ...
        if (paymentMode === "Bank Account" &&
            ...
            return response(res, 404, "Required Field all the AccountNumber,ifscode,bankName for paymentMode Bank Account");
        }
        const uploadPromises = files.map((file) => uploadToCloudinary(file.buffer));
        const results = await Promise.all(uploadPromises);
        const imageUrls = results.map((item) => item.secure_url);
...
export async function getAllProduct(req, res, next) {
    try {
        const products = await Product.find()
            .select("  _id title  author  price  finalPrice  createdAt  condition  category  classType  images")
            .lean();
...
export async function getProductBySellerId(req, res, next) {
    try {
        const userId = req.id;
        if (!userId) {
